...
function drawGameOver() {
    ...
    ctx.fillStyle = 'white';
    ctx.fillText('GAME OVER, your score is: ' + score, canvas.width/2 + 2, canvas.height/2 + 1)
    ctx.fillStyle = "black";
    ctx.fillText("GAME OVER, your score is: " + score, canvas.width/2, canvas.height/2);
}

window.addEventListener('click', (e) => {
    const detectcPixelColor = collisionCtx.getImageData(e.offsetX, e.offsetY, 1, 1);
    let pixelColors = detectcPixelColor.data;
    // let missedHit = [];
    ravens.forEach((raven) => {
        if(raven.randomColors[0] === pixelColors[0] && 
